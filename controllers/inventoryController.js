...
export const addFeedsToStock = async (req, res, next) => {
    ...
    try {
        ...
        // Step 2: Fetch the updated record to get item_type and item_name
        const fetchUpdatedRecordQuery = `SELECT item_type, item_name FROM feeds_inv WHERE id = ?`;
        const [updatedRecordRows] = await connection.query(fetchUpdatedRecordQuery, [id]);
        const updatedRecord = updatedRecordRows[0];

        if (!updatedRecord) {
            throw new Error('Updated record not found');
        }

        const { item_type: itemType, item_name: itemName } = updatedRecord;

        // Step 3: Fetch the last record for the same item_type and item_name
        const lastRecordQuery = `
            ...
            LIMIT 1
        `;
        const [lastRecordRows] = await connection.query(lastRecordQuery, [itemType, itemName]);
        const lastRecord = lastRecordRows[0];

        if (lastRecord) {
            const { batch_id, amount_left, amount_consumed } = lastRecord;

            // Step 4: Calculate the new amount_left for the new record
            const newAmountLeft = amount_left - amount_consumed;

            // Step 5: Insert a new record
            const insertQuery = `
                INSERT INTO feeds_inv (batch_id, item_type, item_name, date, amount_left) 
                VALUES (?, ?, ?, ?, ?)
            `;
            const insertValues = [batch_id, itemType, itemName, date, newAmountLeft];
            ...
            throw new Error('No previous record found for the same item type and name');
        }
    } catch (error) {
        await connection.rollback();
...
export const addSupplementsToStock = async (req, res, next) => {
    ...
    try {
        ...
        // Step 2: Fetch the updated record to get item_type and item_name
        const fetchUpdatedRecordQuery = `SELECT item_type, item_name FROM supplements_inv WHERE id = ?`;
        const [updatedRecordRows] = await connection.query(fetchUpdatedRecordQuery, [id]);
        const updatedRecord = updatedRecordRows[0];
        ...
        if (lastRecord) {
            ...
            // Step 5: Insert a new record
            const insertQuery = `
                INSERT INTO supplements_inv (batch_id, item_type, item_name, date, amount_left) 
                VALUES (?, ?, ?, ?, ?)
            `;
            const insertValues = [batch_id, itemType, itemName, date, newAmountLeft];
            await connection.query(insertQuery, insertValues);
...
export const addChicksToStock = async (req, res, next) => {
    try {        
        ...
        const updateQuery = `UPDATE chicks_inv SET amount_left = ?, ready_to_harvest = ?, undersize = ?, sold = ?, mortality = ? WHERE id = ?`;
        const updateValues = [amount_left, ready_to_harvest, undersize, sold, mortality, id];

        // Execute the update query for chicks
        await db.query(updateQuery, updateValues);

        // Fetch the last record from chicks_inv to get its values
        const lastRecordQuery = `SELECT * FROM chicks_inv WHERE item_name = ? AND batch_id = ? ORDER BY id DESC LIMIT 1`;
        const [rows] = await db.query(lastRecordQuery, [item_name, batch_id]);
        ...
        if (lastRecord) {
            ...
            const insertQuery = `
                ...
                (batch_id, item_type, item_name, date, amount_left, ready_to_harvest, undersize, sold, mortality) 
...
//         // Step 2: Update the selected row with the provided values
//         const updateQuery = `
//             UPDATE chicks_inv 
//             SET date = ?, amount_left = ?, ready_to_harvest = ?, sold = ?, undersize = ?, mortality = ? 
//             WHERE id = ?
//         `;
//         await db.query(updateQuery, [date, amount_left, ready_to_harvest, sold, undersize, mortality, id]);
//         console.log(`Selected row with id: ${id} updated successfully.`);
...
//             // Get the last row from previous rows to calculate the new amount_left for the current row
//             const lastRow = previousRows[previousRows.length - 1];
//             console.log(`Fetched last row with id: ${lastRow.id}, amount_left: ${lastRow.amount_left}, sold: ${lastRow.sold}, mortality: ${lastRow.mortality}`);

//             // Calculate the new amount_left for the selected row (current row)
//             const newAmountLeft = lastRow.amount_left - lastRow.sold - lastRow.mortality;
//             console.log(`New amount_left for row with id: ${id}: ${newAmountLeft}`);
...
//         // Step 5: Adjust the amount_left in all subsequent rows based on the current row's updated amount_left
//         let previousAmountLeft = amount_left;  // Start with the updated amount_left of the current row
//         let previousSold = sold;  // Start with the updated sold of the current row
//         let previousMortality = mortality;  // Start with the updated mortality of the current row

//         for (const row of subsequentRows) {
//             console.log(`Processing row with id: ${row.id}, current amount_left: ${row.amount_left}, sold: ${row.sold}, mortality: ${row.mortality}`);

//             // Calculate new amount_left based on the previous row's amount_left minus the sold and mortality
//             const newRowAmountLeft = previousAmountLeft - previousSold - previousMortality;
//             console.log(`New amount_left for row with id ${row.id}: ${newRowAmountLeft}`);
...
//             // Update previous values for the next row in the loop
//             previousAmountLeft = newRowAmountLeft;
//             previousSold = row.sold;  // This row's sold becomes the previous one in the next iteration
//             previousMortality = row.mortality;  // This row's mortality becomes the previous one in the next iteration
//         }

...
export const addTransaction = async (req, res, next) => {
    ...
    try {
        ...
        if (!transactionType) {
            const error = new Error('Transaction Type is required.');
            error.status = 400;
            return next(error);
        }
        if (!itemType) {
        ...
        if (!pricePerUnit) {
            const error = new Error('Price per Unit is required.');
            error.status = 400;
            return next(error);
        }
        ...
        // Check if the transaction type is 'Expense' and item type is 'Chicks', 'Feeds', or 'Supplements'
        if (transactionType === "Expense" && (itemType === "Chicks" || itemType === "Feeds" || itemType === "Supplements")) {
